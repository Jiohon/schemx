...
| 字段                  | 类型                                                                  | 必填 | 说明                                                                                                                                                                                                                                                                           |
| --------------------- | --------------------------------------------------------------------- | ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `key`                 | `string`                                                              | 否   | ViewSchema 的稳定标识。未设置时 Core 会生成 `key`，并直接回写到传入的 Schema 对象；已显式设置时保留原值。业务代码通常不必设置。                                                                                                                                                |
...
| 字段            | 类型                                                                                   | 必填 | 说明                                                                     |
| --------------- | -------------------------------------------------------------------------------------- | ---- | ------------------------------------------------------------------------ |
| `key`           | `string`                                                                               | 否   | 未设置时由 Core 生成并回写到传入的 Dependency Schema；已设置时保留原值。 |
| `componentType` | `"dependency"`                                                                         | 是   | 固定值。                                                                 |
| `to`            | `TNames`                                                                               | 是   | 触发重新执行 `renderer` 的字段路径列表。                                 |
...
`CreateFormOptions<TValues, TName>` 的所有字段均可选。类型中包含 11 个字段默认配置，但当前规范化阶段只有 `readonly`、`disabled`、`contentAlign`、`validationTrigger` 会读取表单级传值；其余字段仍应直接写在字段 Schema 上。
...
| `required`            | `boolean`                                                                                           | 类型接受该字段，但当前不会作为字段默认值生效。字段未设置时仍按字段 `rules` 和内置默认值推断。                             |
| `readonly`            | `boolean`                                                                                           | 当前会作为字段默认只读状态生效；字段自身配置优先。                                                                        |
| `disabled`            | `boolean`                                                                                           | 当前会作为字段默认禁用状态生效；字段自身配置优先。                                                                        |
| `visible`             | `boolean`                                                                                           | 类型接受该字段，但当前不会作为字段默认值生效；字段未设置时仍使用内置默认值 `true`。                                       |
| `labelIcon`           | `string`                                                                                            | 类型接受该字段，但当前不会作为字段默认值生效。                                                                            |
| `labelAlign`          | `"left" \| "center" \| "right"`                                                                     | 类型接受该字段，但当前不会作为字段默认值生效。                                                                            |
| `labelPosition`       | `"left" \| "top" \| "right"`                                                                        | 类型接受该字段，但当前不会作为字段默认值生效。                                                                            |
| `labelWidth`          | `string`                                                                                            | 类型接受该字段，但当前不会作为字段默认值生效。                                                                            |
| `contentAlign`        | `"left" \| "center" \| "right"`                                                                     | 当前会作为字段默认内容对齐方式生效；字段为只读时会强制使用 `right`。                                                      |
| `validationTrigger`   | `ValidationTrigger \| ValidationTrigger[]`                                                          | 当前会作为字段默认校验触发时机生效；字段自身配置优先。                                                                    |
| `colon`               | `boolean`                                                                                           | 类型接受该字段，但当前不会作为字段默认值生效。                                                                            |
...
`schemas` 省略时等价于空列表，`initialValues` 省略时等价于空对象。对于当前支持的 4 个表单级默认字段，字段 Schema 的显式值优先。
...
| 成员                | 签名                                                                                                                                      | 说明                                                                                                                    |
| ------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- |
| `setSchemas`        | `(schemas: readonly SchemxField<TValues>[]) => void`                                                                                      | 替换根 Schema。已有字段的值、错误与 touched 按字段路径保留；移除字段的相关资源会释放。                                  |
| `updateSchemas`     | `(updater: (schemas: readonly SchemxField<TValues>[]) => readonly SchemxField<TValues>[]) => void`                                        | 基于当前根 Schema 派生下一版。                                                                                          |
...
| 成员                 | 签名                                                                                                                                                                                                                                                                                                                                                                 | 说明                                                                                                                                                                                        |
| -------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `updateDefaultProps` | `(partial: { required?: boolean; readonly?: boolean; disabled?: boolean; visible?: boolean; labelIcon?: string; labelAlign?: "left" \| "center" \| "right"; labelPosition?: "left" \| "top" \| "right"; labelWidth?: string; contentAlign?: "left" \| "center" \| "right"; validationTrigger?: ValidationTrigger \| ValidationTrigger[]; colon?: boolean }) => void` | 合并类型允许的默认配置，并使后续字段规范化读取当前支持的字段。当前只有 `readonly`、`disabled`、`contentAlign`、`validationTrigger` 会被读取；其余字段虽然可传入，但不会作为字段默认值生效。 |
...
复杂 Schema 与表单字段已在「Schema 完整语法」「CreateFormOptions」「表单实例 API」中逐字段说明。下面补充根入口其他类型的结构与用途；完整的 61 个公开类型还会在「完整导出清单」中逐项列出。
...
Core 根入口当前共有 81 个命名导出：20 个运行时值和 61 个类型。包没有默认导出；`createForm` 是命名导出，必须使用 `import { createForm } from "@schemx/core"`。
