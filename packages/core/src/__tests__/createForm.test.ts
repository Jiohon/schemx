...
describe("Runtime type upgrade public behavior regression", () => {
  it("公开的 renderer/rule 快捷方法与 internal hooks 保持一致", () => {
    const form = createForm({
      rendererRegistry: new RendererRegistry(),
      rulesRegistry: new RulesRegistry(),
    })
    const renderer = { component: "Input" }
    const rule = createMockStandardSchema("public-rule")

    form.registerRenderer("input", renderer)
    form.registerRule("publicRule", rule)

    expect(form.hasRenderer("input")).toBe(true)
    expect(form.getRenderer("input")).toBe(renderer)
    expect(form.getInternalHooks().getRenderer("input")).toBe(renderer)

    expect(form.hasRule("publicRule")).toBe(true)
    expect(form.getRule("publicRule")).toBe(rule)
    expect(form.getInternalHooks().getRule("publicRule")).toBe(rule)

    form.destroy()
  })

  it("getResolvedSchemas/getSchemas/waitForDependencies 公开入口保持可用", async () => {
    const form = createForm({
      initialValues: { mode: "show" },
      schemas: [
        {
        ...
        } as any,
        {
          ...
          to: ["mode"],
          renderer: async (values: { mode: string }) => {
            if (values.mode !== "show") return []
    ...
    const resolvedSchemas = form.getResolvedSchemas()

    expect(form.getSchemas()).toEqual(resolvedSchemas)
    expect(
      resolvedSchemas.some((schema) => "name" in schema && schema.name === "detail")
    ).toBe(true)
