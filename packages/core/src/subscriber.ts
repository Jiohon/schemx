...
/** 变化前的表单全量值快照（只读） */
export type PrevSnapshot<T> = T
/** 变化后的表单全量值快照（只读） */
export type LatestSnapshot<T> = T
...
export class Subscriber<T extends FormValues> {
  ...
  private notifyFieldFields(
    ...
    for (const [, { paths, callbacks }] of this.fieldsSubscribers) {
      ...
      if (hasIntersection) {
        const changedValues = pickByPaths(latestSnapshot, paths) as Partial<T>

        const prevValues = pickByPaths(prevSnapshot, paths) as Partial<T>
  ...
  private notifyGlobal(
    ...
    const prevValues = pickByPaths(prevSnapshot, new Set([...changedPaths])) as Partial<T>
