...
import { createFieldInitScheduler } from "../fieldInitScheduler"

describe("createFieldInitScheduler", () => {
  it("多次 batch 在 microtask 中合并为一次 flush", async () => {
    const flushFn = vi.fn()
    const scheduler = createFieldInitScheduler<string>({ flush: flushFn })
  ...
  it("dedupKey 去重，同 key 只保留最后一次", async () => {
    const flushFn = vi.fn()
    const scheduler = createFieldInitScheduler<{ name: string; value: number }>({
      flush: flushFn,
      dedupKey: (t) => t.name,
    })
  ...
  it("size 返回当前待处理任务数（无去重）", () => {
    const scheduler = createFieldInitScheduler<string>({ flush: vi.fn() })
