 ...
 * 避免 SchemaBaseColumnUnion 因 `keyof CustomRendererMap` 为 `never` 而坍塌。
 *
 * 消费者通过声明合并注册真实渲染器类型后，`__stub` 只是联合类型中多出的一个成员，
 * 不影响实际的 componentType 类型收窄。
 */
import type { CustomRendererMap } from "@schemx/core"
import type { CustomRuleMap } from "@schemx/core"
