 ...
 *
 * 提供表单上下文的创建与消费能力。
 * schemx 通过 createFormContext 注入全局配置，
...
export interface FormContextProps {
  /** 表单字段配置 */
  schemas?: SchemxProps["schemas"]
  /** 是否只读 */
  readonly?: SchemxProps["readonly"]
  /** 是否禁用 */
  disabled?: SchemxProps["disabled"]
  /** 标签图标 */
  labelIcon?: SchemxProps["labelIcon"]
  /** 表单 label 排列方向 */
  labelAlign?: SchemxProps["labelAlign"]
  /** 表单 label 位置 */
  labelPosition?: SchemxProps["labelPosition"]
  /** 表单 label 宽度 */
  labelWidth?: SchemxProps["labelWidth"]
  /** 表单内容排列方向 */
  contentAlign?: SchemxProps["contentAlign"]
  /** 是否在 label 后面添加冒号 */
  colon?: SchemxProps["colon"]
  /** 默认的验证触发时机 */
  validationTrigger?: SchemxProps["validationTrigger"]
  /** 自定义类名 */
  className?: SchemxProps["className"]
  /** 自定义样式 */
  style?: SchemxProps["style"]
  /** HTTP 请求器（由 schemx props.request 注入） */
  request?: SchemxProps["request"]
}
