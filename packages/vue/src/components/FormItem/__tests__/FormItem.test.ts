 ...
 * FormItem 集成测试
 *
 * 验证 FormItem 组件与 useResolvedProps 的集成行为：
 * - 新格式 Dependency_Object 正确传递给 useResolvedProps 并驱动动态渲染
...
describe("FormItem 集成测试", () => {
  ...
  it("新格式 dependencies 传递给 useResolvedProps，visible 随依赖字段变化", async () => {
    ...
    // 等待 debounce（16ms）+ nextTick 让 useResolvedProps 完成初始解析
