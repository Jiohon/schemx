...
import type { UploadFileItem, UploadFileType, UploadProps } from "@wot-ui/ui/components/wd-upload/types"
 ...
 * 上传文件对象
 *
 * 描述单个上传文件的状态信息，兼容 Wot UI Upload 的文件格式。
 */
export interface UploadFile extends /* @vue-ignore */ Partial<UploadFileItem> {
  /** 文件 URL */
  url: string
  /** 原始 File 对象 */
  file?: File
...
export interface UploadRendererProps
  extends
    ...
    /* @vue-ignore */
    Partial<Omit<UploadProps, "fileList" | "onUpdate:fileList">> {
  /** 已上传的文件列表 */
  value?: UploadValue
  /** 文件列表变化回调 */
