...
Wot UI renderers for schemx.

`@schemx/wot` 是基于 Vue 3 和 Wot UI 的 Schema 表单组件。导入该包时会自动注册默认 renderer，适合在 uni-app/Wot UI 生态中快速构建移动端动态表单。

## 特性

- 开箱即用的 Vue 3 + Wot UI 表单渲染。
...
```bash
pnpm add @schemx/wot @wot-ui/ui vue
```
