...
import { createFormStore, IFormStore } from '../../core/FormStore'
 ...
 */
function createMockFormContext(store: IFormStore, overrides?: Partial<UseFormReturn>): UseFormReturn {
...
describe('useField Hook (Task 11.2)', () => {
  describe('基础功能 - Requirement 8.2', () => {
    ...
    it('应该支持嵌套路径', () => {
      ...
      const { getField, wrapper } = createTestComponent('user.address.city', {}, { store })
    ...
    it('应该在没有 FormContext 时使用初始值', () => {
      const { getField, wrapper } = createTestComponent('name', { initialValue: 'Default' })
  ...
  describe('touched 状态（字段是否被修改）', () => {
    it('应该在值改变时变为 touched', async () => {
      const store = createFormStore({ initialValues: { name: 'John' } })
      const { getField, wrapper } = createTestComponent('name', { initialValue: 'John' }, { store })
