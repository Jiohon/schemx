...
describe("createRequiredSchema", () => {
  const schema = createRequiredSchema("此字段为必填项")
  ...
  it("undefined 校验失败", () => {
    ...
    expect(result).toEqual({ issues: [{ message: "此字段为必填项" }] })
  })
  ...
  it("不同 errorMessage 生成不同提示", () => {
    const schema2 = createRequiredSchema("请输入邮箱")
