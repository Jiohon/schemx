 ...
 * registerRule、unregisterRule、getFieldError、setFieldError、
...
describe("Validator", () => {
  describe("registerRule / unregisterRule", () => {
    it("注册规则后可校验", async () => {
      ...
      v.registerRule("name", createMinLengthSchema(2, "至少2个字符"))
    ...
    it("registerRule 带 defaultMessage 空值拦截", async () => {
      const v = new Validator<TestForm>()
      v.registerRule("name", createMinLengthSchema(2, "至少2个字符"), "姓名不能为空")
  ...
  describe("validateField", () => {
    ...
    it("支持路径数组批量校验", async () => {
      const v = new Validator<TestForm>()
      v.registerRule("name", createMinLengthSchema(2, "姓名至少2个字符"))
      v.registerRule("email", createRequiredSchema("邮箱不能为空"))
    ...
    it("支持异步校验规则", async () => {
      const v = new Validator<TestForm>()
      v.registerRule("name", createAsyncSchema(10, false, "异步校验失败"))
    ...
    it("null 值触发 defaultMessage", async () => {
      const v = new Validator<TestForm>()
      v.registerRule("name", createMinLengthSchema(1, "至少1个字符"), "必填")
  ...
  describe("validate（全量校验）", () => {
    it("所有字段通过时返回 ok", async () => {
      const v = new Validator<TestForm>()
      v.registerRule("name", createMinLengthSchema(1, "至少1个字符"))
      v.registerRule("email", createRequiredSchema("邮箱不能为空"))
    ...
    it("部分字段失败时返回所有错误", async () => {
      const v = new Validator<TestForm>()
      v.registerRule("name", createMinLengthSchema(10, "姓名至少10个字符"))
      v.registerRule("email", createRequiredSchema("邮箱不能为空"))
