...
const CalendarRendererComponent = defineComponent({
  ...
  setup(props, { attrs, slots }: SetupContext) {
    ...
    // Note: handleCancel is kept for potential future use
...
/**
 * CalendarRenderer - 使用 createRenderWrapper 工厂函数创建的日历选择器渲染器
 *
 * 通过 createRenderWrapper 包装 CalendarRendererComponent，提供：
 ...
 */
// 导出原始组件（用于向后兼容）
export { CalendarRendererComponent }

// 默认导出原始组件以保持向后兼容性
export default CalendarRendererComponent

// 导出包装后的渲染器（用于 RendererRegistry 注册）
export const CalendarRendererWrapped = createRenderWrapper({
  component: CalendarRendererComponent,
  defaultProps: {
    align: "right",
  },
})
